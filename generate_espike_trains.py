...
for frame_no, response_no in tqdm(zip(frame_nos, response_nos), total=frame_nos.size):
    ...
    for estim_trial in tqdm(np.arange(estim_trials)):
        ...
        # Save the plot
        plt.savefig(f"/Volumes/Scratch/Users/ajphillips/tmp/{piece}/figs/frame_{frame_no}_trial_{estim_trial}.png")

        # Close the plot
        plt.close()

    # Save the numpy array
    np.save(f"/Volumes/Stream/Analysis/RETINA/{piece}/spikes/frame_{frame_no}_estim_espont_responses.npy", np.array(estim_espont_responses, dtype=object), allow_pickle=True)